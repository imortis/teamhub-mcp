...
export function fetchHubContext() {
  ...
  return [
    ...
    "",
    "Tell the user which repo this is (the `activeRepo` field below) the",
    "first time it comes up, so a wrong one gets caught early. If they say",
    "it's wrong or they want a different project, call set_repo.",
    "",
    "As you work: call declare_task before starting something new (check",
    "the conflicts, recentActivityNearby and blockedBy it returns),",
    "claim_task to take an existing one, get_diff_for_task before writing a",
    "completion so it reflects what actually changed, update_task_status",
    "with a structured `completion` when done (not a one-line summary),",
    "record_file_note when you finish touching a file,",
    "check_file_before_edit before editing a shared-interface file in a",
    "long session, and update_plan if requirements or design changed.",
    "",
    "```json",
    response.trim(),
    "```",
